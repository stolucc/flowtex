...
function SearchPanel({ view, onClose, projectFiles, onGoToFile, setSearchHighlightEffect }) {
  ...
  const highlightMatch = (text, q, cs) => {
    ...
    return (
      <>
        {text.slice(0, idx)}
        <mark className="search-result-highlight">{text.slice(idx, idx + q.length)}</mark>
    ...
    <div className="editor-search-panel">
      <div className="editor-search-toolbar">
        <div className="editor-search-scope">
          <button
            className={`editor-search-scope-btn ${scope === 'file' ? 'active' : ''}`}
            onClick={() => setScope('file')}
          >
            Current File
          </button>
          <button
            className={`editor-search-scope-btn ${scope === 'tex' ? 'active' : ''}`}
            onClick={() => setScope('tex')}
          >
            .tex Files
          </button>
          <button
            className={`editor-search-scope-btn ${scope === 'all' ? 'active' : ''}`}
            onClick={() => setScope('all')}
          >
            All Files
          </button>
        </div>
        <button className="editor-search-close" onClick={onClose} title="Close (Esc)">
          &times;
        </button>
      </div>
      <div className="editor-search-row">
        ...
        {!isGlobal && (
          <>
            <button className="editor-search-btn" onClick={() => goToMatch('prev')} title="Previous (Shift+Enter)">
              &#x25B2;
            </button>
            <button className="editor-search-btn" onClick={() => goToMatch('next')} title="Next (Enter)">
              &#x25BC;
            </button>
          </>
        )}
      ...
      {!isGlobal && showReplace && (
        <div className="editor-search-row">
          ...
          />
          <button className="editor-search-btn" onClick={handleReplace} disabled={matchIndex < 0} title="Replace">
            Replace
          </button>
          <button
