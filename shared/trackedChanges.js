...
// shipping source to a local helper binary, so the local PDF matches the
// server PDF byte-for-byte for the same input).
//
// These three functions are pure: input string + tc_marks JSONB → output
// string. No I/O, no Express/CodeMirror imports — that's deliberate so
// they're safe to load from both Node (server) and Vite (client) without
// bundle bloat.
//
// In the M2 client model (see TRACK-CHANGES-RULES.md §1.1), the `content`
// column on `files` keeps every char including ones marked-deleted; the
// strikethrough is purely visual. For LaTeX compilation we need either:
//
//   - the "Final" view → strip pending-del ranges (stripPendingDeletions)
//   - the "All Markup" view → wrap pending ranges with \TCadd / \TCdel
//     (wrapPendingChangesAsMacros), preceded by injectTcMacros to add
//     the providecommand block to the preamble
//
...
// entries are either kept-and-marked (markup view) or stripped (final view).
