...
- **PDF viewer** — built-in viewer with zoom, page navigation, click-to-source sync, and an icon-based error / warning chip surfacing compile + lint diagnostics. The pane clears on a failed compile rather than showing a stale PDF
- **One-click error fixes** — diagnostic rows offer a **Fix** button for known remedies: add a missing `\usepackage{...}` (covering ~600 commands plus a helper-LLM fallback), remove an uninstalled package, rename a mismatched environment end, add a `\graphicspath` or swap an image extension, or insert a `.bib` skeleton / open Zotero for an undefined citation. Applied edits are flagged with a glow + ✓ gutter mark you can tick off. Errors with no built-in fix get an **Explain** button that streams a plain-language cause + fix from your local helper LLM. Cascading follow-on errors collapse under their root
...
- **Local LaTeX compile (opt-in)** — install `flowtex-helper` on your own machine (macOS .dmg / Windows .exe / Linux binary) and FlowTex offloads compile from the shared server to your laptop's TeX Live. Each project can override the user-level default and pin a specific TeX Live year. Pre-built helper binaries ship from GitHub Releases, ad-hoc-signed on Mac to satisfy Gatekeeper. The fallback is transparent — if the helper is offline or doesn't have the requested TeX Live year, compile silently falls back to the server
- **Local LLM writing assistant (opt-in)** — once the helper is paired and a local Ollama runtime is running, right-click selected text in the editor for five LLM-driven actions: **Write to length** (rewrite to N words), **Paraphrase**, **Itemize** (prose → `\begin{itemize}`), **Write it out** (bullets → prose), and **Other…** (free-form instruction, hard-locked to textual transformations only). Tokens stream into a preview pane; Accept replaces the selection in place. Selected text and the model output **never leave your machine** — the helper validates that the Ollama URL is loopback-only on every request. Help → Helper setup guide walks you through install + pair + Ollama + the right-click menu
