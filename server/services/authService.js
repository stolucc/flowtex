...
export function decryptTotpSecret(encrypted) {
  ...
  try { return decrypt(encrypted); } catch { return encrypted; }
}
...
async function isTotpUsed(userId, code) {
  ...
  const row = await db.get('SELECT 1 FROM used_totp_codes WHERE user_id = $1 AND code = $2 AND expires_at > NOW()', [userId, code]).catch(() => null);
  return !!row;
}
...
async function markTotpUsed(userId, code) {
  ...
  await db.run("INSERT INTO used_totp_codes (user_id, code, expires_at) VALUES ($1, $2, NOW() + INTERVAL '90 seconds') ON CONFLICT DO NOTHING", [userId, code]).catch(() => {});
}
...
export async function registerUser(email, name, password) {
  ...
  if (existing) throw Object.assign(new Error('Unable to create account. Please try a different email or log in.'), { status: 409 });

  const id = uuid();
  const password_hash = await bcrypt.hash(password, 12);
  await db.run('INSERT INTO users (id, email, name, password_hash) VALUES ($1, $2, $3, $4)', [id, normalizedEmail, name.trim(), password_hash]);
  return { id, email: normalizedEmail, name: name.trim(), totpEnabled: false, isAdmin: false };
}

export async function authenticateUser(email, password) {
  const normalizedEmail = email.toLowerCase().trim();
  const user = await db.get('SELECT id, email, name, password_hash, totp_enabled, totp_secret, is_admin FROM users WHERE email = $1', [normalizedEmail]);
...
export async function checkTrustedDevice(userId, trustCookie) {
  ...
  const device = await db.get('SELECT id FROM trusted_devices WHERE token_hash = $1 AND user_id = $2 AND expires_at > NOW()', [tokenHash, userId]);
  return !!device;
}
...
export async function getCurrentUser(userId) {
  ...
  if (!user) return null;
  return { id: user.id, email: user.email, name: user.name, totpEnabled: !!user.totp_enabled, isAdmin: !!user.is_admin };
}
...
export async function setupTotp(userId) {
  ...
  const totp = new OTPAuth.TOTP({ issuer: 'FlowTex', label: user.email, secret, algorithm: 'SHA1', digits: 6, period: 30 });
...
export async function createPasswordResetToken(email) {
  ...
  const recentTokens = await db.get(`SELECT COUNT(*) AS cnt FROM password_reset_tokens WHERE user_id = $1 AND created_at > NOW() - INTERVAL '1 hour'`, [user.id]);
  if (parseInt(recentTokens?.cnt || 0) >= 3) return null;

  const token = crypto.randomBytes(32).toString('hex');
  const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
  await db.run(`INSERT INTO password_reset_tokens (id, user_id, token_hash, expires_at) VALUES ($1, $2, $3, NOW() + INTERVAL '1 hour')`, [uuid(), user.id, tokenHash]);
...
export async function resetPassword(token, newPassword) {
  ...
  const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
  const resetToken = await db.get(`UPDATE password_reset_tokens SET used = TRUE WHERE token_hash = $1 AND used = FALSE AND expires_at > NOW() RETURNING id, user_id`, [tokenHash]);
  ...
  await db.run('UPDATE password_reset_tokens SET used = TRUE WHERE user_id = $1 AND id != $2', [resetToken.user_id, resetToken.id]);
...
export async function changeEmail(userId, password, newEmail) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)) throw Object.assign(new Error('Invalid email address'), { status: 400 });

  const user = await db.get('SELECT id, email, password_hash FROM users WHERE id = $1', [userId]);
  if (!user) throw Object.assign(new Error('User not found'), { status: 401 });
  if (!(await bcrypt.compare(password, user.password_hash))) throw Object.assign(new Error('Incorrect password'), { status: 401 });
  if (normalizedEmail === user.email) throw Object.assign(new Error('New email is the same as your current email'), { status: 400 });
...
export async function changePassword(userId, currentPassword, newPassword) {
  ...
  if (!user) throw Object.assign(new Error('User not found'), { status: 401 });
  if (!(await bcrypt.compare(currentPassword, user.password_hash))) throw Object.assign(new Error('Current password is incorrect'), { status: 401 });
  if (currentPassword === newPassword) throw Object.assign(new Error('New password must be different from your current password'), { status: 400 });
...
export async function deleteAccount(userId, password) {
  const user = await db.get('SELECT id, email, password_hash FROM users WHERE id = $1', [userId]);
  if (!user) throw Object.assign(new Error('User not found'), { status: 401 });
  if (!(await bcrypt.compare(password, user.password_hash))) throw Object.assign(new Error('Invalid password'), { status: 401 });
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.run(`DELETE FROM projects WHERE id IN (
