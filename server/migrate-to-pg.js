  ...
  try {
    ...
    for (const u of users) {
      await client.query(
        ...
        [u.id, u.email, u.name, u.password_hash, u.totp_secret || null, !!u.totp_enabled, u.created_at || new Date().toISOString()]
      );
    }
    ...
    for (const p of projects) {
      await client.query(
        ...
        [p.id, p.name, p.main_file || 'main.tex', !!p.archived, !!p.trashed, p.created_at || new Date().toISOString(), p.updated_at || new Date().toISOString()]
      );
    }
    ...
    for (const f of files) {
      await client.query(
        ...
        [f.id, f.project_id, f.path, f.content, !!f.is_binary, f.created_at || new Date().toISOString(), f.updated_at || new Date().toISOString()]
      );
    }
    ...
    for (const c of comments) {
      await client.query(
        ...
        [c.id, c.file_id, c.from_pos, c.to_pos, c.text, c.author, c.author_id || null, !!c.resolved, c.created_at || new Date().toISOString()]
      );
    }
    ...
    } catch (e) { console.log('No github_tokens table, skipping.'); }
    ...
    try {
      ...
      for (const l of links) {
        await client.query(
          ...
          [l.project_id, l.github_repo, l.default_branch, l.last_sync_at || null, l.last_sync_commit || null, l.linked_by, l.created_at || new Date().toISOString()]
        );
      }
    } catch (e) { console.log('No project_github_links table, skipping.'); }
    ...
    } catch (e) { console.log('No project_invitations table, skipping.'); }
    ...
    } catch (e) { console.log('No tags table, skipping.'); }
    ...
    try {
      ...
      for (const pt of ptags) {
        await client.query(
          ...
          [pt.project_id, pt.tag_id]
        );
      }
    } catch (e) { console.log('No project_tags table, skipping.'); }
    ...
    try {
      ...
      for (const v of versions) {
        await client.query(
          ...
          [v.id, v.file_id, v.project_id, v.file_path, v.content, v.author_id || null, v.author_name, v.created_at || new Date().toISOString()]
        );
      }
    } catch (e) { console.log('No file_versions table, skipping.'); }
