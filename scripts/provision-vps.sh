...
# ── Configure later (or via the Admin Dashboard) ──────────────────────
# SMTP_HOST=
# SMTP_PORT=587
# SMTP_SECURE=false
# SMTP_USER=
...
Next steps:
  ...
  2. Configure SMTP (email verification / password reset) — edit $APP_DIR/.env
     or use Admin Dashboard > Settings, then: systemctl restart flowtex
